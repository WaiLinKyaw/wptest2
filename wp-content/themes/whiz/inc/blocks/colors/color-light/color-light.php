...
add_action('customize_register', 'whiz_light_color');

function whiz_light_color($wp_customize)
{
    $wp_customize->add_setting('light_color', array(
        'default'     => whiz_get_default_light_color(),
        'transport'   => 'postMessage',
        'sanitize_callback' => 'sanitize_hex_color'
    ));

    $wp_customize->add_control(new WP_Customize_Color_Control($wp_customize, 'light_color', array(
        'label'      => esc_html__('Light Color', 'whiz'),
        'section'    => 'colors',
...
add_action('customize_preview_init', 'whiz_light_color_enqueue_scripts');
function whiz_light_color_enqueue_scripts()
{
